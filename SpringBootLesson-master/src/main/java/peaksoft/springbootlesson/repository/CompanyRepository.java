...
public interface CompanyRepository extends JpaRepository<Company,Long> {
    @Query("select c from Company c where upper(c.companyName) like concat('%',:text,'%') or upper(c.directorName) " +
            "like concat('%',:text,'%') or  upper(c.locatedCountry) like concat('%',:text,'%') ")
